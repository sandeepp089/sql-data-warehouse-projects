...
BEGIN
	...
	BEGIN TRY
		...
		-- data cleaning / standardization marital status values to readable format
		CASE WHEN TRIM(UPPER(cst_material_status)) = 'S' then 'Single'
			 WHEN TRIM(UPPER(cst_material_status)) = 'M' then 'Married'
			 ELSE 'n/a'
		END cst_material_status,
		-- data cleaning / standardization gender values to readable format
		...
		*,
		...
		prd_nm,
		COALESCE(prd_cost,0) prd_cost, -- Handling missing information
		...
		CASE WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid,4, LEN(cid)) -- Remove 'NAS' prefix if present
			 ELSE cid
		END AS cid,
		CASE WHEN TRY_CONVERT(DATE,bdate,105) < '1924-01-01' OR TRY_CONVERT(DATE,bdate,105) > GETDATE()
				THEN NULL
		...
		CASE 
			  ...
			  ELSE 'n/a'
		END AS gen -- Standardized Gender values and handle unknown cases
		...
		SELECT
		 REPLACE(cid,'-','') AS cid, -- Remove Invalid Value
	 ...
	 BEGIN CATCH
	 PRINT '================================================';
	 PRINT 'ERRROR OCCURED DURING LOADING SILVER LAYER';
